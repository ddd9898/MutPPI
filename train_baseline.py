...
if __name__ == '__main__':
    ...
    #Output name
    add_name = '_Train-S4169_ReductionMode{}-{}_RandomSeed-{}'.format(args.RM, args.reduction, args.randomseed)
    #
    ...
    traindataset = StructureGraphDataset(dataname='S4169', augment = AUGMENT, reduction=args.reduction, reduction_mode=args.RM)
    testdataset  = StructureGraphDataset(dataname='S645', augment = False, reduction=args.reduction, reduction_mode=args.RM)
    ...
    model = baseline(in_channels=20).to(device)
    ...
    for epoch in range(NUM_EPOCHS):
        #Train
        train_loss = train(model, device, train_loader, optimizer, epoch, 0)
        
        #Test
        test_loss, g,p = predicting(model, device, test_loader, 0)
