...
from utils.dataloader_CV import StructureGraphDataset, my_graph_collate_fn
...
if __name__ == '__main__':
    ...
    #Output name
    add_name = '_CV{}-Train-S645_ReductionMode{}-{}_RandomSeed-{}'.format(args.fold, REDUCTION_MODE, args.reduction, args.randomseed)
    #
    ...
    traindataset = StructureGraphDataset(datapath='./data/Graphinity/Experimental_ddG_645_+Reverse_Mutations_+Non_Binders/Experimental_ddG_645_+Reverse_Mutations_+Non_Binders-cutoff_random-10foldcv_fixed.csv', train_flag = True, fold = args.fold, reduction=args.reduction, reduction_mode=REDUCTION_MODE)
    testdataset  = StructureGraphDataset(datapath='./data/Graphinity/Experimental_ddG_645_+Reverse_Mutations_+Non_Binders/Experimental_ddG_645_+Reverse_Mutations_+Non_Binders-cutoff_random-10foldcv_fixed.csv', train_flag = False, fold = args.fold, reduction=args.reduction, reduction_mode=REDUCTION_MODE)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if best_result < Pearson_value:
            best_result = Pearson_value
            best_epoch = epoch
            
        logging.info("Best epoch {}: Pearson_value={:.4f}".format(best_epoch, best_result))
