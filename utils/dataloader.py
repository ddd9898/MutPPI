...
class StructureGraphDataset(Dataset):
    def __init__(self, dataname = 'S4169', augment = False, reduction = 10, reduction_mode = 0):
        '''
        ...
        if dataname == 'S4169':
            ##Load structure data
            pos_idx_list = np.load('./data/SKEMPI2/S4169_aa-position.npy', allow_pickle = True).item()
            data = pd.read_csv('./data/SKEMPI2/S4169.csv').values.tolist()
